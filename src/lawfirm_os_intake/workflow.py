...
def run_preflight(
    ...
    append_jsonl(
        ledger_path, _event(run_id, 1, "data_origin_gate", "completed").model_dump(mode="json")
    )
    ...
    append_jsonl(
        ledger_path, _event(run_id, 2, "context_resolution", "completed").model_dump(mode="json")
    )
    ...
    append_jsonl(
        ledger_path,
        _event(run_id, 3, "provenance_preserving_segmentation", "completed").model_dump(
            mode="json"
        ),
    )
    ...
    findings, escalation = review_evidence(parties, matter, deadlines, missing)
    append_jsonl(
        ledger_path, _event(run_id, 4, "specialist_workers", "completed").model_dump(mode="json")
    )
    ...
    packet = IntakePreflightPacket(
        ...
        source_inventory=source_inventory(bundle),
        segments=segments,
    ...
    write_json(run_dir / "source_inventory.json", packet.source_inventory)
...
def build_conflict_seed(
    ...
    classified = set(represented + instructing + payers + adverse + opposing)
