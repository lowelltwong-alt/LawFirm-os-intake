...
def _carrier_compliant_projection_lines(budget: BudgetProposal) -> list[str]:
    ...
    for line in projection.lines:
        if line.capped or line.disallowed or line.staffing_rule_applied:
            capped_refs.append(
                ...
                f"signed_delta={_money(line.line_delta_signed, budget.currency)}, "
                f"review_required={line.requires_human_review}, "
    ...
    return [
        ...
        f"- Signed net delta: {_money(projection.total_delta_signed, budget.currency)}",
        f"- Compliant increase amount: {_money(projection.compliant_increase_amount, budget.currency)}",
        ...
        f"- Signed staffing-rule delta: {_money(projection.staffing_rule_delta_signed, budget.currency)}",
        ...
        f"- Signed blended-rate delta: {_money(projection.blended_rate_delta_signed, budget.currency)}",
        ...
        f"- Review-required projection lines: {projection.review_required_line_count}/{projection.line_count}",
        f"- Projection review issue codes: {', '.join(projection.review_issue_codes) if projection.review_issue_codes else 'none'}",
...
def _budget_precondition_lines(report: BudgetPreconditionReport | None) -> list[str]:
    ...
    lines = [
        ...
        f"- L&E driver impact report ref: `{report.labor_employment_driver_impact_report_ref or 'not supplied'}`",
        f"- L&E driver impact status: {report.labor_employment_driver_impact_status or 'not_applicable'}",
        f"- L&E driver allowed budget output: {report.labor_employment_driver_allowed_budget_output or 'not_applicable'}",
        f"- L&E driver amount-budget block impacts: {report.labor_employment_driver_block_amount_budget_impact_count}",
        f"- L&E driver range-widening impacts: {report.labor_employment_driver_range_widening_impact_count}",
        f"- L&E driver scenario-fork impacts: {report.labor_employment_driver_scenario_fork_impact_count}",
        f"- L&E driver rate/guideline-review impacts: {report.labor_employment_driver_rate_guideline_review_impact_count}",
        f"- L&E driver max range-widening factor: {report.labor_employment_driver_max_range_widening_factor}",
        f"- External writes performed: {report.external_writes_performed}",
...
def _budget_line_lines(budget: BudgetProposal) -> list[str]:
    ...
    for line in budget.lines:
        ...
        basis_refs = ", ".join(line.estimate_basis_refs) or "none"
        lines.append(
            ...
            f"estimate basis: {line.estimate_basis}; estimate refs: {basis_refs}; "
            f"source evidence: {_refs_text(line.evidence_refs, limit=3)}"
