...
def test_review_package_completeness_fails_on_deadline_docketing_guard_drift(tmp_path, repo_root):
    budget_dir = _run_budget(tmp_path, repo_root)
    manifest, safety_report, exception_readiness_report, review_path = _load_inputs(budget_dir)
    guard_path = Path(manifest.artifact_refs["preflight_deadline_docketing_guard_report"])
    guard_report = load_json(guard_path)
    guard_report["docketing_action_performed"] = True
    ...
    assert report.status == "failed"
    assert any(
        check.check_id == "deadline_docketing_guard_report_complete" and check.status == "failed"
        for check in report.checks
    )
    with pytest.raises(ValueError, match="deadline_docketing_guard_report_complete"):
        enforce_review_package_completeness(report)
...
def test_review_package_completeness_fails_on_context_boundary_drift(tmp_path, repo_root):
    budget_dir = _run_budget(tmp_path, repo_root)
    manifest, safety_report, exception_readiness_report, review_path = _load_inputs(budget_dir)
    context_path = Path(manifest.artifact_refs["preflight_context_boundary_report"])
    context_report = load_json(context_path)
    context_report["practice_context_is_observed_evidence"] = True
    ...
    assert report.status == "failed"
    assert any(
        check.check_id == "context_boundary_report_complete" and check.status == "failed"
        for check in report.checks
    )
    with pytest.raises(ValueError, match="context_boundary_report_complete"):
        enforce_review_package_completeness(report)
