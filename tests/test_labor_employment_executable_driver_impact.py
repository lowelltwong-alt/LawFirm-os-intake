...
def test_labor_employment_executable_driver_impact_maps_drivers_to_budget_effects(
    ...
    assert discrimination_messy.allowed_budget_output == (
        "candidate_range_after_review_pending_human_review"
    )
    assert discrimination_messy.impact_item_count == 7
    ...
    discrimination_messy_impacts = {
        item.driver_dimension: item for item in discrimination_messy.impact_items
    }
    assert "add_scenario_fork" in (
        discrimination_messy_impacts["administrative_exhaustion"].impact_actions
    )
    assert "add_scenario_fork" in (discrimination_messy_impacts["forum_arbitration"].impact_actions)
    ...
    assert restrictive_messy.allowed_budget_output == "range_or_hours_only_pending_review"
    assert restrictive_messy.impact_item_count == 6
    ...
    restrictive_messy_impacts = {
        item.driver_dimension: item for item in restrictive_messy.impact_items
    }
    assert "add_scenario_fork" in restrictive_messy_impacts["forum_arbitration"].impact_actions
    ...
    assert class_clean.critical_review_only_impact_count == 3
    assert class_clean.scenario_fork_impact_count >= 3
    class_messy = cases["le-class-collective-messy-thread.executable.v0_1"]
    assert class_messy.allowed_budget_output == "range_or_hours_only_pending_review"
