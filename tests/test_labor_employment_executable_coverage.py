...
def test_labor_employment_executable_coverage_reports_partial_pack_coverage(
    ...
    assert persisted.executable_fixture_count == 14
    assert persisted.executable_pack_case_link_count == 15
    assert persisted.covered_pack_case_count == 15
    assert persisted.missing_executable_pack_case_count == 17
    assert persisted.covered_family_count == 8
    assert persisted.missing_family_count == 0
    assert persisted.covered_family_variant_count == 15
    assert persisted.missing_family_variant_count == 17
    ...
    assert "Missing executable pack cases: 17" in notes
...
def test_labor_employment_executable_coverage_cli_writes_candidate_report(
    ...
    assert report["missing_executable_pack_case_count"] == 17
