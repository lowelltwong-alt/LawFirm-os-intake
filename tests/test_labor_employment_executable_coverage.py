...
def test_labor_employment_executable_coverage_reports_partial_pack_coverage(
    ...
    assert persisted.executable_fixture_count == 15
    assert persisted.executable_pack_case_link_count == 16
    assert persisted.covered_pack_case_count == 16
    assert persisted.missing_executable_pack_case_count == 16
    assert persisted.covered_family_count == 8
    assert persisted.missing_family_count == 0
    assert persisted.covered_family_variant_count == 16
    assert persisted.missing_family_variant_count == 16
    ...
    assert family["ada_fmla_accommodation_leave"].covered_case_count == 3
    assert family["ada_fmla_accommodation_leave"].missing_variants == ["adversarial"]
    ...
    assert family["epli_carrier_assignment"].covered_case_count == 3
    assert family["epli_carrier_assignment"].missing_variants == ["adversarial"]
    ...
    assert "Missing executable pack cases: 16" in notes
...
def test_labor_employment_executable_coverage_cli_writes_candidate_report(
    ...
    assert report["missing_executable_pack_case_count"] == 16
