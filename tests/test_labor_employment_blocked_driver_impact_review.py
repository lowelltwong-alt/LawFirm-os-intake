...
def test_labor_employment_blocked_driver_impact_review_explains_blockers(
    ...
    assert persisted.blocked_case_count == 16
    assert persisted.nonblocking_case_count == 15
    assert persisted.blocker_fact_count == 33
    ...
    assert {
        ...
        "party_topology",
        "representation_posture",
    } <= set(retaliation_adversarial.critical_driver_dimensions)
    assert {
        "employee_claimant_identity",
        "employer_or_defendant_identity",
        "prospective_client_payer_carrier_posture",
        "relevant_employment_timeline",
    } <= {fact.fact_id for fact in retaliation_adversarial.blocker_facts}
    assert retaliation_adversarial.block_amount_budget_impact_count == 4
    assert all(
        fact.fact_resolution_state == "missing_critical_fact"
        for fact in retaliation_adversarial.blocker_facts
    )
    ...
    assert {
        ...
        "party_topology",
        "representation_posture",
    } <= set(admin_adversarial.critical_driver_dimensions)
    assert {
        "employee_claimant_identity",
        "employer_or_defendant_identity",
        "prospective_client_payer_carrier_posture",
        "relevant_employment_timeline",
    } <= {fact.fact_id for fact in admin_adversarial.blocker_facts}
    assert admin_adversarial.block_amount_budget_impact_count == 4
    assert all(
        fact.fact_resolution_state == "missing_critical_fact"
        for fact in admin_adversarial.blocker_facts
    )
...
def test_labor_employment_blocked_driver_impact_review_cli_writes_packet(
    ...
    assert exit_code == 0
    assert report["status"] == "labor_employment_blocked_driver_impacts_ready_for_review"
    assert report["blocked_case_count"] == 16
    assert report["nonblocking_case_count"] == 15
    assert report["blocker_fact_count"] == 33
    assert report["block_amount_budget_impact_count"] == 33
