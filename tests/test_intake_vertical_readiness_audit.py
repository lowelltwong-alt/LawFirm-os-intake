...
def test_intake_vertical_readiness_audit_marks_pr_review_ready_but_not_promoted(
    ...
    assert persisted.implemented_slice_count == persisted.total_slice_count == 30
    le_slice = next(
        slice_status
        for slice_status in persisted.slices
        if slice_status.title == "Labor/employment budget fact-gap audit"
    )
    assert le_slice.status == "implemented_local_candidate"
    assert "audit-labor-employment-budget-facts" in le_slice.command_refs
    assert le_slice.missing_artifact_refs == []
    assert le_slice.missing_command_refs == []
...
def test_intake_vertical_readiness_audit_blocks_failed_or_missing_learning_chain(
    ...
    assert report.implemented_slice_count == report.total_slice_count == 30
    assert any(
