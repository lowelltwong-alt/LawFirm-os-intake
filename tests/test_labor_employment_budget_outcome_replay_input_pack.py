...
def test_labor_employment_budget_replay_input_pack_marks_ready_and_missing_inputs(
    ...
    assert report.ready_input_count == 47
...
def test_labor_employment_budget_replay_input_pack_cli_writes_report(
    ...
    assert '"ready_input_count": 47' in captured.out
...
def test_labor_employment_replay_input_pack_rejects_swapped_discrimination_actuals_source(
    repo_root,
    tmp_path,
):
    manifest = load_json(_input_pack_manifest(repo_root))
    for entry in manifest["entries"]:
        if (
            ...
            entry["input_ref"] = (
                "examples/synthetic/labor-employment/replay-inputs/"
                "wage-hour-clean/budget_actuals_source.json"
            )
    manifest_path = write_json(
        tmp_path / "swapped-discrimination-actuals-input-pack-manifest.json",
        manifest,
    )
    ...
    report, _ = run_labor_employment_budget_outcome_replay_input_pack_audit(
        ...
        input_pack_manifest_path=manifest_path,
        repo_root=repo_root,
        out_dir=tmp_path / "le-budget-outcome-replay-input-pack-swapped-discrimination-actuals",
        generated_at="2026-07-04T00:00:00Z",
    )
    discrimination_case = next(
        case
    ...
    assert report.status == "blocked_by_labor_employment_budget_replay_input_pack"
    assert discrimination_case.status == "blocked"
    assert actuals_item.input_status == "invalid"
    assert actuals_item.validation_model == "BudgetActualsSource"
    assert "source case token='discrimination-harassment-clean'" in (
        actuals_item.validation_message
    )
    assert "actuals_source_id" in actuals_item.validation_message
    assert "source_ref" in actuals_item.validation_message
    assert "le-actuals-wage-hour-clean.v0_1" in actuals_item.validation_message


def test_labor_employment_replay_input_pack_rejects_swapped_discrimination_carrier_bundle(
    repo_root,
    tmp_path,
):
    manifest = load_json(_input_pack_manifest(repo_root))
    for entry in manifest["entries"]:
        if (
            ...
            entry["input_ref"] = (
                "examples/synthetic/labor-employment/replay-inputs/"
                "wage-hour-clean/carrier_rejection_capture_source_bundle.json"
            )
    manifest_path = write_json(
        tmp_path / "swapped-discrimination-carrier-input-pack-manifest.json",
        manifest,
    )
    ...
    report, _ = run_labor_employment_budget_outcome_replay_input_pack_audit(
        ...
        input_pack_manifest_path=manifest_path,
        repo_root=repo_root,
        out_dir=tmp_path / "le-budget-outcome-replay-input-pack-swapped-discrimination-carrier",
        generated_at="2026-07-04T00:00:00Z",
    )
    discrimination_case = next(
        case
    ...
    assert "bundle_id" in carrier_item.validation_message
    assert "run_id" in carrier_item.validation_message
    assert "le-carrier-rejection-wage-hour-clean.v0_1" in carrier_item.validation_message
