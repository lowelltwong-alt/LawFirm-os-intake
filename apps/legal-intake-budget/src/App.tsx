...
function SyntheticGuidelineProjectionWorkbenchPanel({
  ...
  return (
    <section className="panel guideline-projection-workbench-panel" aria-labelledby="guideline-projection-workbench-title">
      ...
      <div className="budget-input-metrics" aria-label="Synthetic output-language split">
        <article><span>Work-Plan Total</span><strong>{formatMoney(view.projection.projection_report?.work_plan_total ?? view.projection.proposed_total)}</strong><p>immutable baseline; not overwritten by reimbursement math</p></article>
        <article><span>Guideline-Adjusted Reimbursement</span><strong>{formatMoney(view.projection.projection_report?.guideline_adjusted_reimbursement ?? null)}</strong><p>candidate; not carrier-approved</p></article>
        <article><span>Unreimbursed Exposure</span><strong>{formatMoney(view.projection.projection_report?.unreimbursed_exposure ?? null)}</strong><p>work-plan minus reimbursement</p></article>
      </div>
      <div className="table-wrap budget-input-table-wrap"><table><thead><tr><th>Phase / Task</th><th>Proposed Role</th><th>Projected Role</th><th>Proposed</th><th>Projection</th><th>Net Delta</th><th>Reason</th></tr></thead><tbody>
        {view.projection.lines.filter((line) => line.line_delta_signed !== 0).map((line) => <tr key={line.line_id}><td><strong>{line.phase_id} / {line.task_id}</strong></td><td>{line.staffing_role.replaceAll("_", " ")}</td><td>{(line.compliant_staffing_role ?? line.staffing_role).replaceAll("_", " ")}</td><td>{formatMoney(line.proposed_line_total)}</td><td>{formatMoney(line.compliant_line_total)}</td><td>{formatMoney(line.line_delta_signed)}</td><td>{line.note}</td></tr>)}
      </tbody></table></div>
