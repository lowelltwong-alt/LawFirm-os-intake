...
The run also writes `model_adapter_report.json` for the selected adapter. `structured-model` is currently dry-run only: zero provider calls, zero external tools, no network or external writes, typed JSON-only requirements, prompt hashes, deterministic baseline authority, independent critic, and human gates. It requires `--fixture-gold` and fails closed if reviewed synthetic gold is missing or failing; a passing run records typed-JSON validation, deterministic baseline projection hash comparison, dry-run structured candidate hash, and fixture-gold status.
