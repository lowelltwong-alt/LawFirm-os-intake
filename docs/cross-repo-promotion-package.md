...
- Budget scenario set, driver effect, driver profile summary, guideline flag, named timekeeper rate, carrier-compliant projection, leverage summary, carrier preapproval, budget-form mapping, budget review change, budget revision report, budget actuals source, phase/code actual comparison, variance-driver candidate, and budget exception-mapping contracts.
...
- Reviewed learning gate report contract for candidate learning pressure from carrier rejections, human budget revisions, and actual variance, with required human-reviewed outcome evidence, append-only evidence, synthetic fixture update, shadow eval, owning-repo review, and no mutation flags.
...
- Intake vertical readiness audit contract for final local slice status, generated artifact-chain validation, PR-review readiness, external adoption requirements, and no-promotion/no-connector/no-Lake/no-sibling-write boundaries.
