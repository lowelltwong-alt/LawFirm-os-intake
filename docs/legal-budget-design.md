...
The local `audit-labor-employment-executable-coverage` command compares the executable fixture manifest to the full L&E fixture-family pack and emits `labor_employment_executable_coverage_report.json`. It records covered and missing pack cases by family/variant so the QA dashboard can show exactly where executable synthetic source bundles still need to be created. Partial executable coverage is a transparent QA state, not authorization to skip missing families.
