...
The `audit-intake-vertical-readiness` command consumes `learning_owner_handoff_report.json`, `budget_event_lake_admission_bundle_report.json`, `budget_calibration_readiness_report.json`, and `budget_fixture_update_review_report.json`, checks the local candidate slices, generated learning artifact chain, budget-event Lake bundle, calibration-readiness chain, and fixture-update review record, and writes `intake_vertical_readiness_audit_report.json` plus `intake_vertical_readiness_audit_report.md`. A passing report means ready for human PR review, not ready for production: it keeps `pr_marked_ready=false`, `promotion_authorized=false`, `proposed_changes_applied=false`, `no_connector_implemented=true`, `no_lake_admission_performed=true`, `no_sibling_repo_writes=true`, and `no_canonical_mutation=true`.
