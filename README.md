...
budget coherence, synthetic fixture depth, budget calibration readiness, the
labor/employment QA matrix, and the L&E fixture family pack into
`synthetic_qa_bundle_report.json` before refreshing the UI manifest. The bundle
may be honestly `blocked` when calibration, L&E matrix, or L&E pack evidence has
...
The pack is spec-only synthetic data with 32 candidate cases across eight L&E
budget families and four variants per family: clean, messy-thread,
missing-attachment, and adversarial. The audit writes
`labor_employment_fixture_family_pack_report.json`, checking family/variant
coverage, configured fact-needs coverage, budget-driver dimensions, blocked and
range-only cases, adversarial holdout posture, and no-write/no-calibration
boundaries. It does not generate runtime fixtures, ingest public records, write
...
The `build-budget-actual-variance-owner-adoption` command consumes `budget_actual_comparison_report.json` and `budget_actual_variance_ledger_report.json`, then writes `budget_actual_variance_owner_adoption_report.json`, Markdown notes, packet JSONL, and owner-specific packets for Semantic Substrate, Orchestrator, and Exception Lake. The packets separate semantic label review, governed billing-actuals workflow review, and append-only Lake admission review while preserving no billing connector read/write, no sibling repo write, no Lake/SQLite write, no budget/profile/template/guideline mutation, and no silent learning.
...
The `prove-cross-repo-contract` command turns that request into a local three-repo regression proof. It requires clean, explicitly supplied Orchestrator and Exception Lake worktrees, invokes their merged no-write validators, and writes `cross_repo_contract_proof_report.json` with the owner commit IDs and artifact hashes. A pass still means `blocked_pending_owner_review` and `blocked_pending_exception_lake_owner_review`; it proves interface compatibility only and never authorizes a connector, Lake/SQLite admission, budget submission, matter opening, conflict clearance, or client use.

The `review-learning-gate` command aggregates candidate learning pressure from carrier rejection learning reports, human budget revisions, and budget actual variance reports. It writes `reviewed_learning_gate_report.json`, `reviewed_learning_gate_report.md`, and `reviewed_learning_gate_candidates.jsonl`, and keeps every candidate blocked until human-reviewed outcome evidence, append-only evidence recording, synthetic fixture updates, shadow evals, and owning-repo review exist. Carrier learning reports additionally fail unless every proposal has source-record-bound QRD and CHW request inputs; repeatable `--lesson-disclosure-gate-request` and `--chinese-wall-gate-request` files expose those proof checks without making them authority. A failed report returns a nonzero CLI result. The command performs no profile, template, connector, budget, guideline, Lake, SQLite, or external mutation.

The CAL-DP candidate under `src/lawfirm_os_intake/privacy/` and `src/lawfirm_os_intake/calibration/` exercises a homegrown Gaussian mechanism over clipped synthetic sufficient statistics, quadratic zCDP group accounting, utility refusal, a non-security reconstruction smoke check, and local hash-chained JSONL evidence. It is test machinery only: every policy value must be labeled a synthetic placeholder, calibrated values are excluded, real/private inputs fail closed, production privacy is not claimed, and `reviewed_learning_gate.py` remains the only promotion-review chokepoint. That gate deliberately stays failed for DP until an authority-owned ledger receipt and governed secret-seed service are verified.

The QRD candidate under `src/lawfirm_os_intake/lessons/` converts only pinned synthetic lesson fixtures into a closed `LessonIR`, applies a reviewed synthetic generalization lattice, checks population k-anonymity plus explicit sensitive-outcome diversity, blocks strategy-class atoms and signal-bearing free text, and checks the complete declared publication snapshot for differencing. Its exact claim is only `bounded_reident_under_declared_adversary`, never formal privacy. `reviewed_learning_gate.py` remains the single promotion-review chokepoint, and the gate cannot pass until an authority-owned complete publication snapshot and authenticated human disclosure review exist. It publishes no lesson, crosses nothing to DAD, and admits no real, private, client, matter, carrier, or privileged data.

The IFC candidate under `src/lawfirm_os_intake/outbox/` applies a closed `public < candidate < internal < privileged` label lattice, deterministic declared-pattern residue checks, and a hash-bound crossing proof to a rebuilt, sanitized QRD artifact. It does not send mail or write an outbox. Its check label is only `deterministic_declared_pattern_residue_check`; it claims no formal noninterference or semantic zero-residue guarantee. A clean local scan is only candidate evidence: `reviewed_learning_gate.py` still fails closed because QRD publication authority, DAD receiver-schema authority, authenticated human crossing review, and owning-repo review are absent. Currency/rate, specific carrier, PII, privilege/work-product, free-text, raw/long-source, high-label, real/private, and unknown inputs are prohibited from the candidate crossing surface.

The CHW candidate under `src/lawfirm_os_intake/conflicts/` applies an exact
Brewer-Nash-style relationship check to one pinned synthetic adversity graph
and one pinned case manifest.
Only fixture-reviewed edges can produce a local adverse result; unreviewed and
unknown relationships hold, and no similarity inference or auto-clearance
exists. Synthetic firm-wide provenance snapshots are pinned, while
authoritative firm-wide imputation is explicitly unverified. Proofs expose only
digests and counts. They remain blocked because counsel-owned adversity classes (HD-4),
authenticated human conflicts review, and owning-repo review are all absent.
The code never fires a lesson, clears a conflict, writes Exception Lake, or
handles real client, matter, carrier, private, privileged, or work-product data.
...
The `build-public-synthetic-fixture-pr-package` command consumes `public_synthetic_fixture_conversion_review_outcome_report.json` plus the matching `public_synthetic_fixture_conversion_plan.json`, then writes `public_synthetic_fixture_pr_package_report.json`, `public_synthetic_fixture_pr_package_report.md`, and package-item JSONL when an approved outcome requires a separate fixture-generation PR. It produces manual PR instructions only while keeping `fixture_generation_authorized=false`, `github_pr_created=false`, `fixture_files_mutated=false`, `public_records_ingested=false`, `raw_public_payload_committed=false`, `legal_knowledge_adapter_authorized=false`, `lake_write_performed=false`, `sqlite_write_performed=false`, and `silent_learning_performed=false`.

The `build-public-methodology-owner-handoff` command consumes `public_source_methodology_report.json`, `public_synthetic_fixture_conversion_plan.json`, and `public_synthetic_fixture_conversion_review_packet.json`, then writes `public_methodology_owner_handoff_report.json`, Markdown notes, owner packet JSONL, and per-owner packets for Intake, Legal Knowledge Runtime, Semantic Substrate, Orchestrator, and Exception Lake. It proves the public-methodology chain is ready for owner review only while keeping `github_issue_created=false`, `github_pr_created=false`, `sibling_repo_write_performed=false`, `direct_promotion_performed=false`, `promotion_authorized=false`, `fixture_generation_authorized=false`, `public_records_ingested=false`, `raw_public_payload_committed=false`, `legal_knowledge_adapter_authorized=false`, `lake_write_performed=false`, `sqlite_write_performed=false`, and `silent_learning_performed=false`.
...
The `audit-intake-vertical-readiness` command consumes `learning_owner_handoff_report.json`, `budget_event_lake_admission_bundle_report.json`, `budget_calibration_readiness_report.json`, `budget_fixture_update_review_report.json`, and `budget_fixture_update_pr_package_report.json`, checks the local candidate slices, generated learning artifact chain, budget lifecycle audit surface, budget human-review packet, outcome, outcome-owner-adoption, actual-variance owner-adoption, budget lifecycle owner-adoption packet surfaces, labor/employment budget fact-gap audit surface, budget-event Lake bundle, calibration-readiness chain, fixture-update review record, public conversion review outcome record, public fixture PR package, and fixture-update PR package, and writes `intake_vertical_readiness_audit_report.json` plus `intake_vertical_readiness_audit_report.md`. A passing report means ready for human PR review, not ready for production: it keeps `pr_marked_ready=false`, `promotion_authorized=false`, `proposed_changes_applied=false`, `no_connector_implemented=true`, `no_lake_admission_performed=true`, `no_sibling_repo_writes=true`, and `no_canonical_mutation=true`.

The `build-budget-human-review-packet` command consumes `budget_lifecycle_audit_report.json` plus optional budget revision, actual comparison, carrier rejection review, and carrier rejection learning reports, then writes `budget_human_review_packet.json`, `budget_human_review_packet.md`, and `budget_human_review_decision_templates.json`. The packet gives human reviewers recommendations, why-notes, red-team notes, required review sections, decision templates, and budget/rejection/actuals financial summary while preserving no budget submission, no appeal submission, no billing write, no Lake/SQLite write, no profile/template/budget/guideline mutation, no sibling repo write, and no silent learning.

The `record-budget-human-review-outcome` command consumes `budget_human_review_packet.json` plus an explicit human outcome JSON, then writes `budget_human_review_outcome_record.json`, `budget_human_review_outcome_history.jsonl`, `budget_human_review_outcome_report.json`, and Markdown notes. It records append-only decisions for correction, appeal, write-off, owner routing, and learning-loop holds while preserving no budget submission, no appeal submission, no billing write, no Lake/SQLite write, no profile/template/budget/guideline mutation, no sibling repo write, and no silent learning.

The `build-budget-human-review-outcome-owner-adoption` command consumes `budget_human_review_outcome_report.json` and the matching `budget_human_review_outcome_record.json`, then writes `budget_human_review_outcome_owner_adoption_report.json`, `budget_human_review_outcome_owner_adoption_report.md`, `budget_human_review_outcome_owner_adoption_packets.jsonl`, and owner-specific packets for Semantic Substrate, Orchestrator, and Exception Lake. It turns human budget decisions into manual owner-review work for outcome labels, runtime followups, and append-only Lake admission while creating no issues or PRs, writing no sibling repos, admitting no Lake/SQLite records, submitting no budgets or appeals, mutating no budgets/profiles/templates/guidelines, and applying no learning.
...
The `record-pr-readiness-decision` command consumes `pr_review_checklist.json`, `intake_local_closeout_report.json`, and an explicit human decision JSON, then writes `pr_readiness_decision_record.json`, `pr_readiness_decision_history.jsonl`, `pr_readiness_decision_report.json`, and Markdown notes. A `mark_ready_for_review` decision means a manual GitHub action is required; it still keeps `pr_marked_ready=false`, `github_write_performed=false`, `github_issue_created=false`, `sibling_repo_write_performed=false`, `lake_write_performed=false`, `sqlite_write_performed=false`, and `silent_learning_performed=false`.
...
The `audit-owner-issue-draft-quality` command consumes `cross_repo_owner_issue_draft_report.json` and writes `owner_issue_draft_quality_report.json` plus Markdown notes. It checks every owner issue draft for required sections, source-evidence labels, red-team notes, acceptance checks, matching markdown output, and explicit no-write/no-promotion/no-learning boundary text before humans copy anything into owner repos. Blocked source drafts remain blocked, and the report preserves `github_issue_created=false`, `github_pr_created=false`, `github_write_performed=false`, `sibling_repo_write_performed=false`, `promotion_authorized=false`, `lake_write_performed=false`, `sqlite_write_performed=false`, `external_writes_performed=false`, and `silent_learning_performed=false`.

The `audit-intake-local-closeout` command consumes the final readiness audit, PR checklist, owner-adoption report, and owner issue-draft report, then writes `intake_local_closeout_report.json` and `intake_local_closeout_report.md`. A passing report means intake-local candidate work is ready for manual external actions: human PR state decision, optional manual owner issue creation, owner triage, owner implementation PRs if accepted, and cross-repo validation after owner changes. `--observed-pr-state` accepts `draft`, `ready_for_review`, `merged`, or `not_supplied` as observed evidence only. It does not mark a PR ready, create issues, open PRs, write sibling repos, promote canon, admit Lake records, write SQLite, apply learning, or authorize production use.

The `record-dad-review-issue` command consumes a structured Fable/Codex/Claude/human review issue JSON record and appends deterministic candidate mail to `.digital-asset/mail/outbox.jsonl` for DAD pickup. The record captures severity, issue classes, candidate exception labels, observable context, observable decision logic, solution path, fix/test/artifact refs, applicability limits, and danger-if-misapplied notes. It suppresses duplicate issues by stable mail ID and dedupe key, rejects sensitive-looking or oversized payloads, captures no hidden chain-of-thought, and performs no Lake/SQLite/external write or learning promotion. See `docs/dad-review-issue-outbox.md`.

The `plan-remaining-roadmap` command consumes `intake_vertical_readiness_audit_report.json`, `intake_local_closeout_report.json`, and optionally `pr_readiness_decision_report.json`, then writes `remaining_roadmap_report.json`, `remaining_roadmap_items.jsonl`, and Markdown notes. It separates easy human/local actions from critical owner-gated work, records next recommended items, acceptance evidence, red-team notes, and remaining gates, and moves the next recommendations to owner follow-up work when the supplied closeout or PR decision evidence records `observed_pr_state=merged`. It preserves `github_issue_created=false`, `github_pr_created=false`, `github_write_performed=false`, `sibling_repo_write_performed=false`, `promotion_authorized=false`, `lake_write_performed=false`, `sqlite_write_performed=false`, `external_writes_performed=false`, and `silent_learning_performed=false`.

The `plan-pr-merge-order` command consumes an explicit local PR snapshot JSON and writes `pr_merge_order_readiness_packet.json` plus Markdown notes. It recommends a manual draft-PR merge queue, identifies shared changed-file surfaces that need rebase attention, and keeps fixture gap closers ahead of the depth audit so the audit can be rebased onto the expanded holdout set. It preserves `ready_for_review_marked=false`, `merge_performed=false`, `github_issue_created=false`, `github_pr_created=false`, `github_write_performed=false`, `sibling_repo_write_performed=false`, `promotion_authorized=false`, `lake_write_performed=false`, `sqlite_write_performed=false`, `external_writes_performed=false`, and `silent_learning_performed=false`.

The `audit-synthetic-fixture-expansion` command consumes `remaining_roadmap_report.json` plus `examples/synthetic/fixture-expansion/remaining-roadmap-holdouts.json`, then writes `synthetic_fixture_expansion_report.json` and Markdown notes. The report proves synthetic holdout coverage for ambiguous roles, missing actuals, carrier rejection variants, and budget driver edge cases, including the labor/employment ready-critical-facts holdout, while preserving `calibration_approved=false`, `fixture_files_mutated_by_audit=false`, `github_write_performed=false`, `sibling_repo_write_performed=false`, `lake_write_performed=false`, `sqlite_write_performed=false`, `external_writes_performed=false`, and `silent_learning_performed=false`.

The `build-skills-registry-specialist-review` command consumes `skill-agent-manifest.json`, `agents/*.yaml`, declared `harnesses/*.yaml`, `prompts/registry.yaml`, and local schema refs, then writes `skills_registry_specialist_review_report.json`, `skills_registry_specialist_review_report.md`, `skills_registry_specialist_candidates.jsonl`, and per-worker files under `skills_registry_specialist_packets/`. It packages the seven predeclared intake specialists for Skills Registry owner review only, verifies prompt hashes, schema refs, and harness refs, checks no network/cross-matter/external-tool authority, and keeps `skill_promoted=false`, `skill_trust_record_created=false`, `dynamic_agent_created=false`, `model_provider_enabled=false`, `real_data_approved=false`, `github_write_performed=false`, `sibling_repo_write_performed=false`, `promotion_authorized=false`, and `silent_learning_performed=false`.
...
Budget proposals also embed local `BudgetDriverEffect` and `BudgetGuidelineFlag` records. Severity, liability, and venue drivers apply bounded synthetic intensity multipliers under the approved baseline-relative policy in `docs/governance/intensity_normalization_signoff.json`; default matter-family tiers normalize to `1.0` so template assumptions are not double-counted. Coverage posture is surfaced as a review boundary; synthetic guideline caps become review flags. Profile defaults are labeled as defaults, never observed facts, and guideline flags do not rewrite budget rates, hours, expenses, or totals.
...
Python remains the starter reference implementation. If future document volume or constrained compute requires Rust, the approved hot-path boundary starts with source inventory, segmentation, hashing, and evidence-ref emission. For public/synthetic dataset preparation, future Rust may also be proposed in shadow mode for deterministic CourtListener snapshot normalization, offline corpus manifest indexing, duplicate detection, source-span indexing, and document-label offset indexing. Any Rust adapter must prove parity with the Python reference for offsets, hashes, segment structure, prompt-injection flags, duplicate/missing-source states, source-span refs, label offsets, and schema-compatible JSON before it can replace any Python path.
