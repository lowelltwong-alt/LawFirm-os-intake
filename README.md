...
The same package now renders authority/precondition checks, source inventory, evidence-graph summary, and run-ledger summary inline. Reviewers can see contract-state status, human-review outcome, budget precondition checks, each source's read/missing/duplicate state, hashes, attachment refs, provenance graph counts and key support edges, and the preflight/budget gate trail before opening the JSON artifacts.
