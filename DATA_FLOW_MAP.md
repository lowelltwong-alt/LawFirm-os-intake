...
| `ExceptionLakeCandidate` in `exception_lake_candidates.jsonl` | Intake candidate surface | Intake -> Orchestrator -> Exception Lake review path | Dry-run only; maps to broad existing Lake classes, includes no raw payload, and may carry source refs, evidence refs, or structured refs; untrusted attempts to clear conflicts, open matters, create workspaces, docket deadlines, submit budgets, or send external messages become specific local prohibited-transition candidates |
...
| `workflow_escalation` | human review required, role ambiguity, contradictory candidates, missing information, prompt-injection source content, specific local prohibited transition attempts, budget blocked before confirmation, budget unknowns, missing budget template, hours-only missing rates | Append escalation trigger, evidence refs, structured workflow refs, and current blocked state |
