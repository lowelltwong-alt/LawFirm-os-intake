...
| Future `CarrierRejectionNotice` / `CarrierRejectionRemediationCase` / `CarrierAppealResult` | Orchestrator + Exception Lake future runtime; Intake candidate roadmap only | Carrier portal/email/LEDES/manual source -> Orchestrator -> Exception Lake; Intake supplies fixtures/evals only | Proposed deterministic capture loop for 100% carrier budget/invoice rejection reconciliation. Every submitted artifact needs a response state; unlinked notices, parser failures, duplicates, stale responses, rejection classes, human fix/appeal decisions, appeal results, recovered/write-down amounts, and learning candidates should be append-only Lake evidence. See `docs/carrier-rejection-learning-loop-roadmap.md` |
